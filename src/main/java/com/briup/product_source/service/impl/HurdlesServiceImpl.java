...
public class HurdlesServiceImpl implements HurdlesService {
    ...
    @Autowired
    private ManagerHurdlesExtMapper hurdlesExtMapper;
    ...
    public PageInfo<ManagerHurdlesExt> findByPage(Integer pageNum, Integer pageSize,
        ...
        //2.执行条件查询
        List<ManagerHurdlesExt> list =
                hurdlesExtMapper.findHurdlesWithHouseByConditions(hName, hMax, fhName, hEnable);

        //3.封装结果并返回
        PageInfo<ManagerHurdlesExt> info = new PageInfo<>(list);
        return info;
    }
